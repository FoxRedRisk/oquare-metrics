...
def generate_metrics_xml_simple(
    basic_metrics: Dict[str, int],
    oquare_metrics: Dict[str, float],
    output_path: Union[str, Path],
    ontology_name: Optional[str] = None
    ...
    """
    Generate a simpler XML format that matches the JAR output more closely.
    This version omits the metadata and sections for closer compatibility.
    ...
    # Add all basic metrics directly to root
    basic_mapping = {
        'numberOfClasses': 'numberOfClasses',
        'numberOfLeafClasses': 'numberOfLeafClasses', 
        'numberOfObjectProperties': 'numberOfObjectProperties',
    ...
    for internal_name, xml_name in basic_mapping.items():
        if internal_name in basic_metrics:
            elem = SubElement(root, xml_name)
            elem.text = str(basic_metrics[internal_name])
    
    # Add all OQuaRE metrics directly to root
    for metric_name, value in oquare_metrics.items():
        elem = SubElement(root, metric_name)
        if isinstance(value, int):
            elem.text = str(value)
        else:
            elem.text = f"{value:.15f}"
    ...
    logger.info(f"Metrics XML (simple format) saved to: {output_path}")
